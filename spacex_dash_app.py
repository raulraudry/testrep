...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        fig = px.pie(site_df,  names='class', title=title)
...
def get_scatter_chart(entered_site,payload_value):
    ...
    filtered_df = spacex_df
    if entered_site == 'ALL':
        filtered_site = filtered_df[filtered_df['Payload Mass (kg)'] <= int(payload_value[1])]
        ...
        filtered_site = filtered_df[(filtered_df['Launch Site'] == entered_site) & 
                                    (filtered_df['Payload Mass (kg)'] <= int(payload_value[1]))]
        fig = px.scatter(filtered_site, x='Payload Mass (kg)', y='class', title=title, color='Booster Version Category')
    return fig
